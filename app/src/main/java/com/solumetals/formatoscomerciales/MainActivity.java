...
public class MainActivity extends Activity {
    ...
    void openDocument(String type){
        ...
        String metaText=type.equals("Membrete")||type.equals("Tarjeta de presentación")?"FECHA "+date:"N.º "+String.format(Locale.getDefault(),"%05d",n)+"     FECHA "+date+(type.equals("Recibo")?"     HORA "+hour:"");TextView meta=text(metaText,15,type.equals("Membrete")?NAVY:Color.RED);meta.setBackground(box(Color.rgb(228,240,245),18));meta.setPadding(20,22,20,22); root.addView(meta);
    ...
    void buildFields(String t){
        ...
        else if(t.equals("Membrete")){TextView drag=text("ARRASTRAR O CARGAR IMAGEN DEL MEMBRETE\nPNG o JPG",15,Color.GRAY);drag.setGravity(Gravity.CENTER);GradientDrawable g=bordered(Color.WHITE,TEAL,24);g.setStroke(3,TEAL,12,8);drag.setBackground(g);form.addView(drag,new LinearLayout.LayoutParams(-1,150));letterheadPreview=new ImageView(this);letterheadPreview.setScaleType(ImageView.ScaleType.CENTER_INSIDE);form.addView(letterheadPreview,new LinearLayout.LayoutParams(-1,220));loadLetterhead();View.OnClickListener pick=v->{Intent i=new Intent(Intent.ACTION_OPEN_DOCUMENT);i.setType("image/*");i.addCategory(Intent.CATEGORY_OPENABLE);startActivityForResult(i,10);};drag.setOnClickListener(pick);letterheadPreview.setOnClickListener(pick);contentField=field("Escriba aquí la carta, constancia o información","",null,true);contentField.setMinLines(14);form.addView(contentField);}
    ...
    void createPdf(boolean share){
        ...
        if(currentType.equals("Membrete")){Bitmap letter=getLetterhead();if(letter!=null){float sh=Math.min(125f,letter.getHeight()*(W-60f)/letter.getWidth());c.drawBitmap(letter,null,new RectF(30,24,W-30,24+sh),p);y=(int)(38+sh);}else{p.setColor(Color.LTGRAY);p.setStyle(Paint.Style.STROKE);p.setStrokeWidth(2);c.drawRoundRect(30,24,W-30,130,12,12,p);p.setStyle(Paint.Style.FILL);p.setTextSize(12);p.setColor(Color.GRAY);c.drawText("MEMBRETE DEL NEGOCIO",48,78,p);y=150;}p.setColor(Color.DKGRAY);p.setTextSize(13);p.setTypeface(Typeface.DEFAULT);y=drawWrapped(c,p,contentField==null?"":contentField.getText().toString(),42,y,W-84);p.setColor(accent);c.drawRect(36,H-48,W-36,H-46,p);p.setColor(Color.GRAY);p.setTextSize(9);c.drawText("Tel: "+safe(prefs.getString("phone",""))+"   RUC: "+safe(prefs.getString("tax","")),42,H-30,p);c.drawText(safe(prefs.getString("email","")),W/2f,H-30,p);
        }else{
            Bitmap bm=getLogo();if(bm!=null){float sc=Math.min(95f/bm.getWidth(),62f/bm.getHeight());c.drawBitmap(bm,null,new RectF(30,y,30+bm.getWidth()*sc,y+bm.getHeight()*sc),p);}p.setColor(accent);p.setTypeface(Typeface.create(Typeface.DEFAULT,Typeface.BOLD));p.setTextSize(card?16:19);c.drawText(safe(prefs.getString("business","Mi negocio")),card?125:150,y+20,p);p.setTypeface(Typeface.DEFAULT);p.setTextSize(card?8:10);c.drawText("Tel: "+safe(prefs.getString("phone",""))+"   RUC: "+safe(prefs.getString("tax","")),card?125:150,y+38,p);c.drawText(safe(prefs.getString("address","")),card?125:150,y+53,p);y=card?105:120;
            p.setColor(accent);c.drawRoundRect(28,y-22,W-28,y+12,10,10,p);p.setColor(Color.WHITE);p.setTextSize(card?14:18);p.setTypeface(Typeface.DEFAULT_BOLD);c.drawText(currentType.toUpperCase(Locale.getDefault()),40,y,p);String date=new SimpleDateFormat("dd/MM/yyyy",Locale.getDefault()).format(new Date());int n=prefs.getInt("seq_"+currentType,1);if(!card){p.setColor(Color.DKGRAY);p.setTextSize(10);p.setTypeface(Typeface.DEFAULT);c.drawText("Fecha: "+date,34,y+32,p);p.setColor(Color.RED);c.drawText("N.º "+String.format(Locale.getDefault(),"%05d",n),W-112,y+32,p);y+=52;}
            if(currentType.equals("Factura")||currentType.equals("Cotización")){y=drawInvoice(c,p,y,W);}else{for(int i=0;i<form.getChildCount();i++){View child=form.getChildAt(i);if(!(child instanceof EditText))continue;EditText e=(EditText)child;String label=e.getHint()==null?"":e.getHint().toString();String val=e.getText().toString();p.setTypeface(Typeface.DEFAULT_BOLD);p.setColor(NAVY);p.setTextSize(9);c.drawText(label,30,y,p);y+=13;p.setTypeface(Typeface.DEFAULT);p.setColor(Color.DKGRAY);p.setTextSize(11);y=drawWrapped(c,p,val,30,y,W-60);p.setColor(Color.LTGRAY);c.drawLine(30,y+2,W-30,y+2,p);y+=18;if(y>H-55)break;}}
        }
        p.setColor(Color.GRAY);p.setTextSize(7);c.drawText("Formatos Comerciales",30,H-12,p);doc.finishPage(page);int n=prefs.getInt("seq_"+currentType,1);
